...
namespace OpenUtau.App.Controls
{
    class WaveformImage : Control
    {
        ...
        private IDisposable? waveformRefreshSubscription;
        private IDisposable? themeChangedSubscription;
        ...
        protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e)
        {
            ...
            waveformRefreshSubscription = MessageBus.Current.Listen<WaveformRefreshEvent>()
                .Subscribe(_ => RequestRedraw());
            themeChangedSubscription = MessageBus.Current.Listen<ThemeChangedEvent>()
                .Subscribe(_ => RequestRedraw());
        }
        ...
        {
            PhraseWaveformCache.Changed -= OnPhraseWaveformCacheChanged;
            waveformRefreshSubscription?.Dispose();
            waveformRefreshSubscription = null;
            themeChangedSubscription?.Dispose();
            themeChangedSubscription = null;
            bitmap?.Dispose();
            bitmap = null;
            bitmapData = Array.Empty<int>();
            ...
            if (bitmap != null)
            {
                    ...
                    viewModel.TickWidth > ViewConstants.PianoRollTickWidthShowDetails)
                {
                    ...
                    if (project != null && part != null)
                    {
                        ...
                        var cachedPhrases = PhraseWaveformCache.GetForTrack(part.trackNo).ToArray();
                        FillFromPhraseCache(cachedPhrases, leftMs, ref needsAnotherFrame);

                        if (part.Mix != null && cachedPhrases.Length == 0)
                        {
                        ...
                        int peakColor = GetWaveformPeakColor();
                        int startSample = 0;
                        for (int i = 0; i < bitmap.PixelSize.Width; ++i)
                        {
                            ...
                            if (endSample > startSample)
                            {
                                ...
                                {
                                    DrawPeak(bitmapData, bitmap.PixelSize.Width, i, (int)Math.Round(yMin), (int)Math.Round(yMax), peakColor);
                                }
                            }
        ...
        void FillFromPhraseCache(PhraseWaveformCache.Entry[] cachedPhrases, double leftMs, ref bool needsAnotherFrame)
        {
            foreach (var cacheItem in cachedPhrases)
            {
        ...
        private WriteableBitmap? GetBitmap()
        {
                ...
                bitmap.PixelSize.Height != desiredHeight)
            {
                ...
                Log.Debug("Created waveform bitmap {BitmapSize}", size);
        ...
        private static void DrawPeak(int[] data, int width, int x, int y1, int y2, int color)
        {
            if (y1 > y2)
            {
                int temp = y2;
