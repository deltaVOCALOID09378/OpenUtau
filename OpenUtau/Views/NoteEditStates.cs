...
namespace OpenUtau.App.Views
{
    ...
    class NoteMarqueeOverrideEditState : NoteEditState
    {
        const double DragThresholdSquared = 16;
        readonly PianoRoll pianoRoll;
        readonly Rectangle selectionBox;
        readonly UNote? clickNote;
        NoteSelectionEditState? marqueeState;

        protected override bool ShowValueTip => false;

        public NoteMarqueeOverrideEditState(
            Control control,
            PianoRollViewModel vm,
            PianoRoll pianoRoll,
            Rectangle selectionBox,
            UNote? clickNote) : base(control, vm, pianoRoll)
        {
        ...
        public override void Update(IPointer pointer, Point point)
        {
            ...
            {
                marqueeState.Update(pointer, point);
                return;
            }
            var delta = point - startPoint;
            if (delta.X * delta.X + delta.Y * delta.Y < DragThresholdSquared)
            {
                return;
            }
            marqueeState = new NoteSelectionEditState(
                control, vm, pianoRoll, selectionBox);
            marqueeState.Begin(pointer, startPoint);
            marqueeState.Update(pointer, point);
        }
        ...
        public override void End(IPointer pointer, Point point)
        {
            if (marqueeState != null)
            {
                marqueeState.End(pointer, point);
                return;
            }
            pointer.Capture(null);
            if (clickNote != null)
            {
                vm.NotesViewModel.ToggleSelectNote(clickNote);
            }
        }
    }
    ...
    class PitchCurveState : NoteEditState
    {
        ...
        /// </summary>
        public bool TransitionToAdjusting(IPointer pointer, Point point)
        {
