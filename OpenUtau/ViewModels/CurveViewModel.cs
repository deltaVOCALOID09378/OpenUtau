...
namespace OpenUtau.ViewModels
{
    ...
    public class CurveViewModel : ViewModelBase, ICmdSubscriber
    {
        ...
        {
            // [DELTA SYNTH] ตั้งค่ารับ Notification เมื่อมีการเลือกตัวโน้ต เพื่อให้ล้างข้อมูลเคอร์ฟที่เลือกไว้ก่อนหน้า
            DocManager.Inst.AddSubscriber(this);
            MessageBus.Current.Listen<NotesSelectionEvent>()
                .Subscribe(e =>
                {
                    if (e.selectedNotes != null && e.selectedNotes.Length > 0)
                    {
                        ClearSelect();
                    }
                });
        ...
        public void Paste(UVoicePart part, UExpressionDescriptor descriptor)
        {
            ...
            // [DELTA SYNTH] Guard: ป้องกันแครชกรณีคลิปบอร์ดว่างเปล่า หรือเป้าหมายไม่สมบูรณ์
            if (part == null || paste == null || descriptor == null || !paste.HasValue(descriptor.abbr)) return;
