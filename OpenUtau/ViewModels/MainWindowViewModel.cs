...
namespace OpenUtau.App.ViewModels
{
    ...
    public class RecentFileInfo
    {
        ...
        {
            // [DELTA SYNTH] ระบบป้องกัน File I/O Error: เช็คชื่อไฟล์และจัดการการอ่านเวลาด้วย Try-Catch
            PathName = path;
            ...
            {
                LastWriteTime = DateTime.MinValue; // กรณีไฟล์ถูกล็อคหรือไม่มีอยู่จริง ให้รีเซ็ตเวลาแทนการแครช
            }
    ...
    public class MainWindowViewModel : ViewModelBase, ICmdSubscriber
    {
        ...
        public ObservableCollectionExtended<MenuItemViewModel> TimelineContextMenuItems { get; } = new ObservableCollectionExtended<MenuItemViewModel>();
        ...
        // [DELTA SYNTH] ควบคุมการแสดงผลชื่อแอปพลิเคชัน
        public string AppVersion => $"Thai OpenUtau By DELTA SYNTH v{System.Reflection.Assembly.GetEntryAssembly()?.GetName().Version}";
        ...
        public GridLength PianoRollSplitterRowHeight => ShowPianoRoll && !PianoRollFullscreen ? new GridLength(8) : new GridLength(0);
        public ReactiveCommand<UPart, Unit> PartDeleteCommand { get; set; }
        ...
        [Reactive] public string UndoText { get; set; } = ThemeManager.GetString("menu.edit.undo") ?? "Undo";
        [Reactive] public string RedoText { get; set; } = ThemeManager.GetString("menu.edit.redo") ?? "Redo";

        private ScreenRecorder _screenRecorder = new ScreenRecorder();
        [Reactive] public bool IsRecording { get; set; } = false;
        [Reactive] public bool IsRecordingPaused { get; set; } = false;
        public ReactiveCommand<Unit, Unit>? RecordCommand { get; private set; }
        public ReactiveCommand<Unit, Unit>? StopRecordCommand { get; private set; }

        private ObservableCollectionExtended<MenuItemViewModel> openRecentMenuItems = new ObservableCollectionExtended<MenuItemViewModel>();
        private ObservableCollectionExtended<MenuItemViewModel> openTemplatesMenuItems = new ObservableCollectionExtended<MenuItemViewModel>();
        ...
        public MainWindowViewModel()
        {
            ...
            // [DELTA SYNTH] กรองไฟล์ประวัติการใช้งาน: ตรวจสอบเฉพาะไฟล์ที่ยังอยู่จริง เพื่อป้องกันข้อผิดพลาดเวลาเริ่มโปรแกรม
            RecentFiles.Clear();
            if (Preferences.Default.RecentFiles != null)
            {
            ...
            {
                Log.Warning(ex, "Failed to load template files on startup.");
            }
            ...
            RecordCommand = ReactiveCommand.Create(() =>
            {
                if (!_screenRecorder.IsRecording) {
                    var outputPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), $"OpenUtau_Record_{DateTime.Now:yyyyMMdd_HHmmss}.mp4");
                    _screenRecorder.StartRecord(outputPath);
                    IsRecording = true;
                    IsRecordingPaused = false;
                } else if (!_screenRecorder.IsPaused) {
                    _screenRecorder.PauseRecord();
                    IsRecordingPaused = true;
                } else {
                    _screenRecorder.ResumeRecord();
                    IsRecordingPaused = false;
                }
            });

            StopRecordCommand = ReactiveCommand.Create(() =>
            {
                if (_screenRecorder.IsRecording) {
        ...
        {
            if (PianoRollFullscreen || !ShowPianoRoll) return;
        ...
        private void SetUndoState()
        {
            ...
            UndoText = string.IsNullOrWhiteSpace(undoNameKey) ? 
                (ThemeManager.GetString("menu.edit.undo") ?? "Undo") : 
                $"{ThemeManager.GetString("menu.edit.undo")}: {ThemeManager.GetString(undoNameKey)}";

            CanRedo = DocManager.Inst.GetRedoState(out string? redoNameKey);
            RedoText = string.IsNullOrWhiteSpace(redoNameKey) ? 
                (ThemeManager.GetString("menu.edit.redo") ?? "Redo") : 
                $"{ThemeManager.GetString("menu.edit.redo")}:  {ThemeManager.GetString(redoNameKey)}";
        }
        ...
        public void ImportAudio(string file)
        {
            ...
            UWavePart part = new UWavePart() { FilePath = file };
            part.Load(project);
        ...
        public void ImportMidi(string file)
        {
            ...
            foreach (var part in parts)
            {
                ...
                if (part.name != "New Part") track.TrackName = part.name;
        ...
        public void RefreshOpenRecent()
        {
            ...
            if (Preferences.Default.RecentFiles != null)
            {
                openRecentMenuItems.AddRange(Preferences.Default.RecentFiles
                    .Where(file => !string.IsNullOrWhiteSpace(file) && File.Exists(file))
                    .Select(file => new MenuItemViewModel()
                    {
                        Header = file,
        ...
        public void RefreshTemplates()
        {
            ...
            {
                Log.Warning(ex, "Could not refresh template menu.");
            }
        }

        public void RefreshCacheSize()
        {
            string header = ThemeManager.GetString("menu.tools.clearcache") ?? "Clear Cache";
        ...
        public void RefreshTimelineContextMenu(int tick)
        {
            ...
            {
                string template = ThemeManager.GetString("context.timeline.deltempo") ?? "Delete Tempo at {0}";
                TimelineContextMenuItems.Add(new MenuItemViewModel
                {
            ...
            {
                string template = ThemeManager.GetString("context.timeline.addtempo") ?? "Add Tempo at {0}";
                TimelineContextMenuItems.Add(new MenuItemViewModel
                {
        ...
        {
            // [DELTA SYNTH] ควบคุมการคำนวณตำแหน่งใหม่ โดยไม่ต้องลบหรือสร้างโน้ตใหม่
            var project = DocManager.Inst.Project;
            foreach (var part in project.parts)
            {
                ...
                if (part is UVoicePart voicePart)
                {
                    ...
                    {
                        DocManager.Inst.ExecuteCmd(new ResizeVoicePartCommand(project, voicePart, partNewDuration - partOldDuration, false));
                    }
