    ...
    public static class Worldline {
        ...
        class SynthSegment {
            ...
            float GetAutoGain(float[] samples, double[] f0, float wavMax, int peakComp) {
                ...
                // Reduce peakComp intensity to prevent noise boosting / crackling
                double compressionPower = peakComp * 0.005; // Reduced from 0.01 for smoother gain control
                float autoGain = (max < 1e-3f) ? 1.0f : (float)Math.Pow(0.5 / max, compressionPower);
                
                // Clamp autoGain to prevent extreme amplification of background noise
                return Math.Min(autoGain, 2.5f);
            }
        }
