    ...
    public class PluginRunner {
        ...
        public async Task Execute(UProject project, UVoicePart part, UNote? first, UNote? last, IPlugin plugin) {
            ...
            try {
                ...
                // Keep the exchange file outside CachePath. CachePath can be cleared from the UI
                // while a plugin is running, and on Windows it may also be synchronized by OneDrive.
                ...
                if (tempFile != null && File.Exists(tempFile)) {
                    try {
                        ...
                        Log.Warning(e, "Failed to clean up legacy plugin temp file {TempFile}", tempFile);
                    }
                }
            }
        }
